...
/* VENDOR, PRODUCT, CUSTOMER, INVOICE, LINE, EMPLOYEE		*/
...
2. An invoice contains one ore more invoice lines. Each invoice line is associated with one invoice.
3. Each invoice line references one product. A product may be found in many inovice lines. 
(You can sell more than one hammer to more than one customer).
4. A vendor may supply many products. Some vendors do not yet supply products. 
For eample, a vendor list may include potential vendors.
5. If a product is vendor-supplied, it is supplied by only a single vendor.
6. Some products are not supplied by a vendor. For example, some products may be produced in-house or 
